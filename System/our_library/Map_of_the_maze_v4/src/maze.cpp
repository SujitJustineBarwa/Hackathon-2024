...
std::vector<cell*> Maze::getWaypoints(int startX, int startY) {
    ...
    while (current->getValue() != 0) {
        ...
        for (cell* neighbor : current->getNeighborCells()) {
            if (neighbor->getValue() < minValue) {
                minValue = neighbor->getValue();
                nextCell = neighbor;
            }
        }
    ...
    path.push_back(current); // Add the starting cell
    ...
    for (cell* waypoint : path) {
        this->path_x.push_back(waypoint->getX());
        this->path_y.push_back(waypoint->getY());
    }
    return path;
}

std::vector<int> Maze::getPathX() const {
    return path_x;
}

std::vector<int> Maze::getPathY() const {
    return path_y;
}
